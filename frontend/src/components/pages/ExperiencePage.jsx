    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        {projects.map((project, idx) => (
          <div
            key={idx}
            className="glass-card p-5 md:p-6 hover:border-primary/50 transition-all duration-300 hover:shadow-xl flex flex-col"
          >
            {/* Ghost number */}
            <span className="font-display text-5xl font-bold text-primary/10 leading-none mb-2 select-none">
            ...
            <div className="space-y-2.5 mt-auto pt-4 border-t border-border/50">
              {project.highlights.map((highlight, hIdx) => (
                <div key={hIdx} className="flex items-start gap-2.5">
                  <span className="text-primary font-sans font-bold text-xs leading-none mt-1.5 flex-shrink-0">▸</span>
                  <p className="font-sans text-xs md:text-sm text-foreground leading-relaxed">
                    {highlight}
                  </p>
                </div>
              ))}
      ...
      {/* CTA */}
      <div className="glass-card p-6 md:p-8 text-center border-l-4 border-primary">
        <p className="font-display text-xl md:text-2xl font-bold text-foreground mb-2">
          Want to see the code?
        </p>
        <p className="font-sans text-sm md:text-base text-muted-foreground mb-5">
